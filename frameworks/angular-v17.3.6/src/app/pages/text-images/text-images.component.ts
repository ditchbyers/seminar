...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, ReviewCardWithImageComponent],
  template: `
    <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
      <h1 class="text-2xl font-bold">Images <span class="text-blue-600">{{ resolution }}</span></h1>
      <div class="flex gap-2 flex-wrap text-sm">
        <a *ngFor="let r of resolutions" [routerLink]="['/text-images', r]" [queryParams]="{table}"
          [class]="resClass(r)">{{ r }}</a>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      <app-review-card-with-image *ngFor="let r of reviews" [review]="r" [resolution]="resolution" />
    </div>
  `,
})
