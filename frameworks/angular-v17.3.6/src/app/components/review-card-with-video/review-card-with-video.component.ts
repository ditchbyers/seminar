...
export class ReviewCardWithVideoComponent implements OnInit {
  @Input({ required: true }) review!: Review;
  @Input({ required: true }) resolution!: string;
  video: VideoMeta | null = null;

  constructor(private api: ApiService) {}
  ngOnInit() { this.api.getVideo(this.resolution).subscribe(v => (this.video = v)); }
  get stars() {
