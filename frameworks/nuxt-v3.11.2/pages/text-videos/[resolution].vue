...
import { VIDEO_RESOLUTIONS } from '~/utils/config';
...
const { data } = await useAsyncData('text-videos', () => getReviews({ table: table.value, limit: 12 }), { watch: [table, resolution] });
if (!VIDEO_RESOLUTIONS.includes(resolution.value)) { throw createError({ statusCode: 404 }); }
</script>
  ...
  <div>
    <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
      ...
      <div class="flex gap-2 flex-wrap text-sm">
        <NuxtLink v-for="r in VIDEO_RESOLUTIONS" :key="r" :to="`/text-videos/${r}?table=${table}`"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <ReviewCardWithVideo v-for="r in data?.data" :key="r.id" :review="r" :resolution="resolution" />
    </div>
  </div>
</template>
