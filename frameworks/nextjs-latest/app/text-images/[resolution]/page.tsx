...
export default async function TextImagesPage({
  params, searchParams,
}: { params: Promise<{ resolution: string }>; searchParams: Promise<{ table?: string; limit?: string; delay?: string }> }) {
  const { resolution } = await params;
  if (!VALID_RESOLUTIONS.includes(resolution)) return notFound();
  const { table = 'hotel_reviews', limit = '30', delay = '0' } = await searchParams;
  const res = await fetch(`${API_BASE}/api/reviews?table=${table}&limit=${limit}&delay=${delay}`, { cache: 'no-store' });
  const { data: reviews, total } = await res.json();

  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <h1 className="text-2xl font-bold">Images <span className="text-blue-600">{resolution}</span> <span className="text-gray-400 text-base font-normal">({total} total)</span></h1>
