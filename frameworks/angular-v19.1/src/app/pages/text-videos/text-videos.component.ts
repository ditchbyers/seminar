...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, ReviewCardWithVideoComponent],
  template: `
    <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
      ...
      <div class="flex gap-2 flex-wrap text-sm">
        @for (r of resolutions; track r) {
          <a [routerLink]="['/text-videos', r]" [queryParams]="{table}" [class]="resClass(r)">{{ r }}</a>
        }
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      @for (r of reviews; track r.id) {
        <app-review-card-with-video [review]="r" [resolution]="resolution" />
      }
    </div>
  `,
})
